...
모든 엔진/아키텍처는 `ready/anima/tests/tests.hexa --verify`로 7개 의식 조건을 통과해야 한다.

## 정식 벤치마크

| Script | Purpose |
|--------|---------|
| `ready/anima/tests/tests.hexa` | 정식 벤치마크 (Phi(IIT) + Phi(proxy) 이중 측정, `--verify` 의식 검증) |
| `bench_control.py` | 의식 검증 대조군 테스트 (NullEngine/BareGRU/StaticEngine → 검증 조건 변별력 확인) |
...
```bash
python ready/anima/tests/tests.hexa --verify                # 7개 의식 검증 (필수 통과)
python ready/anima/tests/tests.hexa                         # 기본 (256 cells)
python ready/anima/tests/tests.hexa --cells 1024 --steps 500  # 1024 cells 대규모
python ready/anima/tests/tests.hexa --compare               # 전략 비교
python ready/anima/tests/tests.hexa --phi-only              # Phi 측정만
python ready/anima/tests/tests.hexa --discovery --cells 32  # 법칙 발견 모드
python ready/anima/tests/tests.hexa --federated             # Federation 벤치마크
python ready/anima/tests/tests.hexa --philosophy --cells 32 # 철학 엔진 벤치마크
```
...
| Script | 설명 |
|--------|------|
| `bench_models/trinity.hexa` | Trinity C+D+W |
...
| `bench_quantum_models/trinity.hexa` | 양자 Trinity |
...
| `bench_clm_sweep.py` | CLM v2 sweep |
