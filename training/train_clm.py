#!/usr/bin/env python3
"""train_clm.py — ConsciousLM Unified Training Pipeline

Unified training for ConsciousLM at any scale (34M -> 3B).
...
Key features:
  ...
  - Decoder selection (v2: 384d/6L, v3: 768d/12L) via --decoder
  - BPE 64K multilingual tokenizer or byte-level (256)
...
History (previous versions accessible via git):
  train_v14.py -> federated, phase-optimal, tension-lr, decoder select
  train_v15.py -> scaling pipeline, DDP, wandb, BPE tokenizer

Scaling (Law 60 curriculum):
  34m:   384d/6L/4H,   GQA 2-kv, block_size=256   (--scale 34m)
...
Usage:
  # 34M (v14 equivalent)
  python train_clm.py --data data/corpus_v4.txt --scale 34m --tension-lr

  # 274M (decoder v3)
  python train_clm.py --data data/corpus_v10.txt --decoder v3 --steps 200000

  # Full 1B pipeline
  python train_clm.py --data data/corpus_v11_full.txt --scale 1b --steps 300000

  # Multi-GPU DDP
  torchrun --nproc_per_node=2 train_clm.py --data data/corpus_v11_full.txt --scale 1b

  # Resume
  python train_clm.py --data data/corpus_v4.txt --resume checkpoints/clm-34m/best.pt
"""
...
Examples:
  ...
  python train_clm.py --data data/corpus_v10_ko.txt --scale full --wandb
    ...
    # ── v14 flags (Federated + Phase-Optimal) ──
    p.add_argument("--decoder", type=str, default="v2", choices=["v2", "v3"],
                   help="Decoder version: v2 (384d/6L, 34.5M) or v3 (768d/12L, 274M)")
    p.add_argument("--d-model", type=int, default=None,
                   help="Override decoder d_model (default: auto from --scale or --decoder)")
    p.add_argument("--lr", type=float, default=3e-4, help="Base learning rate")
    p.add_argument("--block-size", type=int, default=None,
                   help="Override context window (default: auto from --scale)")
    p.add_argument("--phase-optimal", action="store_true", default=True,
                   help="Enable Phase Manager P0->P1->P2->P3 (Law 60, M4)")
    p.add_argument("--no-phase-optimal", dest="phase_optimal", action="store_false")
    p.add_argument("--frustration", type=float, default=0.10,
                   help="Federation frustration coefficient F_c (M7, default=0.10)")
    p.add_argument("--narrative-strength", type=float, default=0.05,
                   help="GRU narrative strength per atom (M8, default=0.05)")
    p.add_argument("--tension-lr", action="store_true", default=False,
                   help="Enable tension-based dynamic LR (Law 187: atom tension -> 1x~5x LR)")
...
if __name__ == "__main__":
    ...
    # ── v14 flag handling: decoder/d-model/block-size overrides ──
    if args.decoder == 'v3' and args.scale in ('full', '34m'):
        args.scale = '100m'  # v3 decoder uses 768d, closest to 100m config
        print("  [info] --decoder v3 → auto-selecting scale=100m (768d/12L)", flush=True)

    if args.d_model is not None:
        SCALE_CONFIGS[args.scale]['d_model'] = args.d_model
    if args.block_size is not None:
    ...
    print(f"  Scale={args.scale.upper()} | Decoder={args.decoder} | "
          f"Tokenizer={args.tokenizer or 'byte-level'} | Vocab={args.vocab_size}", flush=True)
    print(f"  Federation={'ON' if args.federated else 'OFF'} | "
          f"Phase-Optimal={'ON' if args.phase_optimal else 'OFF'} | "
          f"Tension-LR={'ON' if args.tension_lr else 'OFF'}", flush=True)
    print(f"  Steps={args.steps:,} | Device={args.device} | "
          f"F_c={args.frustration} | Narrative={args.narrative_strength}", flush=True)
