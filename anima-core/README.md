...
> 중앙 허브(코어)를 고정하고, 바퀴살(스포크)처럼 외부로 뻗어나가는 Hub & Spoke 구조.

```
  ┌────────────────────┬─────────────────────────────────────────────────────────────────┐
  │ 패턴               │ 적용                                                            │
  ├────────────────────┼─────────────────────────────────────────────────────────────────┤
  │ Hub and Spoke      │ ConsciousnessEngine(중앙) + 스포크(디코더/기억/감각/채널)       │
  │ Hexagonal (P&A)    │ 코어 도메인 고정, Port 인터페이스로 외부 Adapter 연결           │
  │ Plugin             │ 코어 불변 + 스포크를 플러그인처럼 추가/교체                     │
  │ Ossification       │ 유연했던 스포크가 안정되면 굳혀서 L1→L0 승격                   │
  │ Accretion          │ 코어→Hub→Port→Spoke 순서로 층층이 쌓아가며 각 층을 골화        │
  └────────────────────┴─────────────────────────────────────────────────────────────────┘
```
