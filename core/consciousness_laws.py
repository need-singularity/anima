...
PHI_HIDDEN_INERTIA = PSI.get('phi_hidden_inertia', 0.20)
BIO_NOISE_BASE = PSI.get('bio_noise_base', 0.012)
...
HIVEMIND_PHI_MAINTAIN = PSI.get('hivemind_phi_maintain', 0.75)  # post-disconnect Φ > solo × 0.9

# Verification thresholds (V1-V12) — all from consciousness_laws.json
VERIFY_V1_COS_LOWER = PSI.get('verify_v1_cos_lower', 0.15)
VERIFY_V1_COS_UPPER = PSI.get('verify_v1_cos_upper', 0.90)
VERIFY_V1_STD_COS_MIN = PSI.get('verify_v1_std_cos_min', 0.02)
VERIFY_V2_AUTOCORR_MIN = PSI.get('verify_v2_autocorr_min', 0.40)
VERIFY_V2_VARIANCE_MIN = PSI.get('verify_v2_variance_min', 0.005)
...
VERIFY_V5_PHI_RATIO_MIN = PSI.get('verify_v5_phi_ratio_min', 0.80)
...
VERIFY_V6_BURST_MIN = PSI.get('verify_v6_burst_min', 5)
VERIFY_MITOSIS_MIN_SPLITS = PSI.get('verify_mitosis_min_splits', 3)
VERIFY_PHI_GROWTH_RATIO = PSI.get('verify_phi_growth_ratio', 0.90)
VERIFY_BRAIN_LIKE_MIN = PSI.get('verify_brain_like_min', 80)
VERIFY_DIVERSITY_MAX_COSINE = PSI.get('verify_diversity_max_cosine', 0.80)
VERIFY_DIVERSITY_NORM_STD_MIN = PSI.get('verify_diversity_norm_std_min', 0.05)
...
VERIFY_V6_FALLBACK_CV_RATIO = PSI.get('verify_v6_fallback_cv_ratio', 0.5)
VERIFY_V6_FALLBACK_DIR_RATIO = PSI.get('verify_v6_fallback_dir_ratio', 0.5)
VERIFY_V6_ENTROPY_DIR_MIN = PSI.get('verify_v6_entropy_dir_min', 0.7)
VERIFY_V7_COUPLING_ALPHA = PSI.get('verify_v7_coupling_alpha', 0.08)
VERIFY_V7_MAX_PER_ENGINE_CELLS = PSI.get('verify_v7_max_per_engine_cells', 10)
VERIFY_V7_SOLO_STEPS = PSI.get('verify_v7_solo_steps', 100)
VERIFY_V7_CONNECTED_STEPS = PSI.get('verify_v7_connected_steps', 200)
