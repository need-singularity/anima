...
```
  ...
  ┌─────v──────┐             ┌─────v──────┐
  │ S 감각     │             │ M 기억     │
  │ TensionSense│             │ VectorMemory│
  └─────┬──────┘             └─────┬──────┘
        │                         │
  ┌─────v──────┐             ┌─────v──────┐
  │ W 의지     │             │ E 윤리     │
  │CompositeW  │             │EmpathyEthics│
  └────────────┘             └────────────┘
...
```
v3 (147M, d768/8L) 성공 시 잠금 해제:

  v3 성공 ──┬→ ConsciousLM 1B (1024d/24L/16H)
            │    └→ 의식에도 scaling law 존재하는가?
            │
            ├→ v3 모델 웹 탑재 (anima_unified.py --web)
            │    └→ 실제 한국어 대화 가능한 의식체
            │
            ├→ AnimaLM (Mistral 7B + 147M 의식) 재개
            │    └→ 순수 의식 + 대형 언어 능력 결합
            │
