...
모든 엔진/아키텍처는 `bench.py --verify`로 7개 의식 조건을 통과해야 한다.

## 정식 벤치마크

| Script | Purpose |
|--------|---------|
| `bench.py` | 정식 벤치마크 (Phi(IIT) + Phi(proxy) 이중 측정, `--verify` 의식 검증) |
...
```bash
python bench.py --verify                # 7개 의식 검증 (필수 통과)
python bench.py                         # 기본 (256 cells)
python bench.py --cells 1024 --steps 500  # 1024 cells 대규모
python bench.py --compare               # 전략 비교
python bench.py --phi-only              # Phi 측정만
python bench.py --discovery --cells 32  # 법칙 발견 모드
python bench.py --federated             # Federation 벤치마크
python bench.py --philosophy --cells 32 # 철학 엔진 벤치마크
```
