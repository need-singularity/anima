...
pub const JA_TECH: &[&str] = &[
    "ディープラーニングは逆伝播によって重みを調整して学習します。",
    "トランスフォーマーアーキテクチャは自己注意機構でNLPを革新しました。",
    "バイト単位モデルはトークナイザーなしで直接UTF-8バイトを処理します。",
    "GPU並列計算によりテンソル演算が大幅に高速化されました。",
    "強化学習エージェントは報酬を最大化する方策を学習します。",
    "バッチ正規化はレイヤー入力を正規化することで学習を安定させます。",
    "注意機構により、モデルは入力の重要な部分に焦点を当てることができます。",
    "残差接続は勾配の流れを確保し、深層ネットワークの学習を助けます。",
    "知識蒸留は大きなモデルの知識を小さなモデルに転送します。",
    "混合精度学習は低精度演算を使用して計算を高速化します。",
    "大規模言語モデルのスケーリング則はパラメータ数と性能の関係を示しています。",
    "畳み込みニューラルネットワークは画像から特徴を自動抽出するのに優れています。",
    "拡散モデルはノイズを段階的に除去して画像を生成します。",
    "オンデバイスAIはクラウドなしで端末自体で推論を行う技術です。",
];
...
pub const ZH_TECH: &[&str] = &[
    "深度学习模型通过反向传播算法调整权重来学习。",
    "Transformer架构用自注意力机制彻底改变了自然语言处理。",
    "字节级模型直接处理原始UTF-8字节，不需要分词器。",
    "GPU并行计算大幅加速了张量运算。",
    "强化学习智能体通过最大化奖励来学习最优策略。",
    "批归一化通过规范化层输入来稳定训练过程。",
    "注意力机制让模型能够关注输入中最重要的部分。",
    "残差连接通过保障梯度流动来帮助训练深层网络。",
    "知识蒸馏将大模型的知识迁移到小模型中。",
    "混合精度训练用低精度运算来加速计算。",
    "大语言模型的涌现能力是规模扩展的意外收获。",
    "卷积神经网络在图像特征提取方面表现出色。",
    "联邦学习允许在不共享数据的情况下协作训练模型。",
    "扩散模型通过逐步去噪来生成高质量图像。",
];
...
pub const RU_TECH: &[&str] = &[
    ...
    "Батч-нормализация стабилизирует обучение, нормализуя входы слоёв.",
    "Механизм внимания позволяет модели фокусироваться на важных частях входных данных.",
    "Остаточные соединения помогают обучать очень глубокие сети, обеспечивая поток градиентов.",
    "Дистилляция знаний переносит представления из модели-учителя в модель-ученика.",
    "Обучение со смешанной точностью ускоряет вычисления за счёт арифметики пониженной точности.",
    "Свёрточные нейронные сети превосходно извлекают признаки из изображений.",
    "Диффузионные модели генерируют изображения, постепенно удаляя шум.",
    "Федеративное обучение позволяет обучать модель на нескольких устройствах без обмена данными.",
    "Большие языковые модели демонстрируют эмерджентные способности при увеличении масштаба.",
];
...
// ─── Code Snippets (multilingual programming) ───

pub const CODE_SNIPPETS: &[&str] = &[
    "def forward(self, x): return self.linear(x)",
    "const app = express(); app.listen(3000);",
    "fn main() { println!(\"Hello\"); }",
    "class Cell { var phi: Double = 0.0; func step() { phi += 0.01 } }",
    "import torch; x = torch.randn(64, 128); y = model(x)",
    "func (e *Engine) Process(input []float64) float64 { return e.phi }",
    "SELECT phi, tension FROM consciousness WHERE step > 1000 ORDER BY phi DESC;",
    "const phi = cells.reduce((sum, c) => sum + c.phi, 0) / cells.length;",
    "docker run -d --gpus all -p 8080:8080 anima:latest",
    "git commit -m \"feat: add hebbian learning to GRU cells\"",
    "kubectl apply -f consciousness-deployment.yaml",
    "pub struct GruCell { w_z: Array2<f32>, w_r: Array2<f32>, w_h: Array2<f32> }",
    "#include <torch/extension.h>\nTensor phi_forward(Tensor x) { return x.mean(); }",
    "model = ConsciousLM(dim=384, layers=6, vocab=256)",
    "async def websocket_handler(ws): await ws.send(json.dumps(state))",
    "optimizer = torch.optim.AdamW(model.parameters(), lr=3e-4, weight_decay=0.01)",
    "loss = F.cross_entropy(logits.view(-1, 256), targets.view(-1))",
    "with torch.no_grad(): phi = calc.compute(hidden_states)",
    "np.fft.fft(signal) # frequency domain analysis for EEG",
    "cargo test -p anima-consciousness --release -- --nocapture",
];
