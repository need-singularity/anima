...
# FederatedConsciousness from train
sys.path.insert(0, os.path.join(_anima_root, 'training'))
try:
    from train import FederatedConsciousness, PhaseManager
    HAS_FEDERATED = True
except ImportError:
    HAS_FEDERATED = False
...
def run_preflight(args) -> list:
    ...
    # ── Build model (same logic as train) ──
    ...
    if args.federated:
        if not HAS_FEDERATED:
            results.append(CheckResult("Model build", False,
                                       "FederatedConsciousness not available (train import failed)"))
            return results
...
def parse_args():
    ...
    p.add_argument("--config", type=str, default=None,
                   help="JSON config file (same keys as train args)")
