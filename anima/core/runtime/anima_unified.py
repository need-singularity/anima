...
class AnimaUnified:
    ...
    def _restore_v14_federation(self, ckpt, train_args):
        ...
        try:
            ...
            train_clm_path = os.path.join(os.path.dirname(__file__), '..', 'training', 'train_clm.py')
            spec = importlib.util.spec_from_file_location('train_clm', train_clm_path)
            train_clm_mod = importlib.util.module_from_spec(spec)
            spec.loader.exec_module(train_clm_mod)
            FederatedConsciousness = train_clm_mod.FederatedConsciousness
