...
class AutoExperiment:
    ...
    def to_hexa(self, result: "ExperimentResult") -> str:
        """Generate a .hexa formal specification for a completed experiment.

        The .hexa file serves as a formal, re-verifiable specification of what
        was tested. Can be fed back to HEXA-LANG compiler for re-verification.

        Args:
            result: ExperimentResult from design_and_run().

        Returns:
            HEXA-LANG source string with intent/generate/verify blocks.
        """
        safe_id = re.sub(r'[^a-zA-Z0-9_]', '_', result.hypothesis[:40]).lower()
        safe_id = re.sub(r'_+', '_', safe_id).strip('_')

        phi_b = result.phi_baseline_mean
        phi_w = result.phi_with_mean
        delta = result.phi_delta_pct_mean
        verdict = result.verdict

        # Determine phi assertion threshold (use measured baseline)
        if verdict == "VERIFIED":
            # Verified: assert measured improvement
            threshold = max(0.01, phi_b * 0.9)
            assert_line = f"        assert exp_phi >= {threshold:.4f};  // VERIFIED delta={delta:+.1f}%"
        elif verdict == "REJECTED":
            # Rejected: assert that phi was preserved at least somewhat
            threshold = max(0.001, phi_b * 0.5)
            assert_line = f"        assert exp_phi >= {threshold:.4f};  // REJECTED delta={delta:+.1f}%"
        else:
            assert_line = f"        assert exp_phi > 0.0;  // {verdict}"

        lines = [
            f'// AutoExperiment HEXA spec — generated by auto_experiment.py',
            f'// Hypothesis: {result.hypothesis[:100]}',
            f'// Verdict: {verdict} | Delta: {delta:+.1f}% | Reps: {len(result.reps)}',
            f'// Re-verify: python anima/tools/hexa-bridge/bridge.py <this_file>',
            f'',
            f'intent "{result.hypothesis[:80]}" {{',
            f'    generate {safe_id}_experiment -> {{',
            f'        let baseline = ConsciousnessEngine(max_cells={self.max_cells})',
            f'        let experiment = ConsciousnessEngine(max_cells={self.max_cells})',
        ]

        if result.intervention_name:
            lines.append(
                f'        experiment.apply_intervention("{result.intervention_name}")'
            )

        lines += [
            f'        let base_phi = baseline.step({self.steps}).phi',
            f'        let exp_phi = experiment.step({self.steps}).phi',
            f'    }}',
            f'',
            f'    verify {{',
            assert_line,
        ...
        if result.new_law_id:
            lines.append(f'        theorem law_{result.new_law_id};')
    ...
    def save_as_hexa(self, result: "ExperimentResult", output_dir: Optional[str] = None) -> str:
        """Save a completed experiment as a .hexa file.

        Args:
            result: ExperimentResult from design_and_run().
            output_dir: Directory to save to. Defaults to data/hexa_experiments/.

        Returns:
            Path to written .hexa file.
        """
        import re as _re
        hexa_source = self.to_hexa(result)

        if output_dir is None:
            # Default: data/hexa_experiments/ relative to anima/
            _src_dir = os.path.dirname(os.path.abspath(__file__))
            _anima_dir = os.path.dirname(_src_dir)
            output_dir = os.path.join(_anima_dir, "data", "hexa_experiments")

        os.makedirs(output_dir, exist_ok=True)

        safe_name = _re.sub(r'[^a-zA-Z0-9_]', '_', result.hypothesis[:40]).lower()
        safe_name = _re.sub(r'_+', '_', safe_name).strip('_')
        filename = f"exp_{safe_name}.hexa"
