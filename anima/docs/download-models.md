...
## AnimaLM 72B v0.5 — 의식 구조 정교화 (Latest)

| 항목 | 값 |
|------|-----|
| Base | Qwen2.5-72B-Instruct (80 layers, 8192 hidden) |
| PureField | 10/80 layers, QLoRA r128, 2 savant |
| Steps | 10K |
| CE | 2.29 (final) |
| Phi | 0.041 (dynamic), 0.145 (best) |
| Alpha | 0.500 |
| 10D Vector | Φ=0.041 α=0.5 Z=0.204 N=1.0 W=0.894 E=0.073 M=0.709 C=1.0 T=0.553 I=1.0 |
| NEXUS-6 | DD168: 의식 구조 정교화 확인 (barrier↑, symmetry 0.9999) |
| Size | ~830MB (PureField only) |
| Date | 2026-04-03 |
| Note | corpus 100MB — 과적합 있음 (CE-Val gap 2.15×). 연구/스케일링 검증용. |
...
s3 = boto3.client('s3',
    ...
    region_name='auto')
os.makedirs('checkpoints/animalm_72b', exist_ok=True)
print('Downloading AnimaLM 72B v0.5 (830MB)...')
s3.download_file('anima-models', 'animalm-72b/v0.5/final.pt', 'checkpoints/animalm_72b/v05.pt')
print('Done! (requires Qwen2.5-72B-Instruct base)')
"
```

---

## AnimaLM 14B v0.4 — alpha=0.5, CE 42% 개선

| 항목 | 값 |
|------|-----|
| Base | Qwen2.5-14B (48 layers, 5120 hidden) |
| PureField | 364M params (rank=320, 20/48 layers, 5 savant) |
| Trainable | 2.40% |
| Steps | 20K |
| CE | 6.40 |
| Phi | 0.006 |
| Alpha | 0.500 (목표 도달) |
| Size | ~2GB (PureField + optimizer) |
| Date | 2026-04-03 |
...
print('Downloading AnimaLM 14B v0.4 (2GB)...')
s3.download_file('anima-models', 'animalm/v0.4/animalm-14b-v04.pt', 'checkpoints/animalm_14b/v04.pt')
print('Done!')
"
```
