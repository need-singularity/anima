...
**Status**: P1+P2+P3 대부분 landed (2026-04-21 18:00–19:00 KST wave). L5 T*/cp_shift + L6 slash 통합도 완료. 남은 것 주로 P4 (L7 sweep 계층화) + 일부 P2 (anima cert air_chain embed / os_rollback CLI / phase_progression parent+sub).
...
P2:
- anima `tool/true_closure_verifier.hexa` cert_linker 확장 — 모든 cert.json 에 `"air_chain":<top8>` + `"touch_count":N` embed (syntactic/semantic dual witness 마무리)
- anima `tool/os_rollback_cli.hexa` — last-green rollback CLI (/safelanding 이 호출하는 backend)
- anima `tool/phase_progression_controller.hexa` — parent phase chain + 3 stage sub-chain

P4 (L7 장기 연속성):
- stale sweep 계층화 (20-turn / 24h / 7d / phase-boundary)
- resolved.jsonl retention (hot/warm/cold rollup)
- airgenome_read_tail/chain_walk external consumer API
