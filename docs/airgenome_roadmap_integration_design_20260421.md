...
**Status**: design seed — 구현 전 계획 문서 (L0-L7 layered)
...
│   └── airgenome CLI 확장: `airgenome hook {status|activate|touch|release}`
│       → hexa-lang `tool/airgenome_hook.hexa` wrapper (airgenome 없으면 no-op)
│
...
│
├── L6. Claude Code CLI 연동 15 지점 (전부)
│   ├── settings.json hooks
│   │   ├── SessionStart → roadmap ready + open chain surface
│   │   ├── UserPromptSubmit → prompt ↔ chain resolver + scope drift warn
│   │   ├── PreToolUse → integrity_guard --check 4 block
│   │   ├── PostToolUse → integrity_guard 8-check bg + status_flip dry-run
│   │   ├── Stop → stale chain sweep + handover.jsonl
│   │   └── SubagentStop → bg Agent 결과 → parent chain rollup + path rewrite
│   ├── MCP tools (airgenome + 신규 3)
...
│   │   ├── /drill <seed> (chain activate)
│   │   ├── /verify (roadmap_engine verify-gates)
│   │   ├── /advance (status flip with gate)
│   │   └── /safelanding (rollback + status)
│   ├── TaskCreate/Update ↔ chain 1:1 sync
│   ├── Transcript watcher → 이탈 키워드 매칭
│   ├── ScheduleWakeup/CronCreate → watcher 이중화 (launchd down 시 fallback)
│   ├── PushNotification → severity 기반 alert
│   ├── Plans mode → plan.md = chain seed, 업데이트=touch
│   ├── Worktree → AIRGENOME_HOOK_ROOT swap + merge rollup
│   └── hook chain trail (.hook-chain)
