...
### 진행 중 (in-flight) — P4 (L7 장기 연속성) 3-agent 병렬 wave

- [agent 1] stale sweep 계층화 (20-turn micro / 24h daily / 7d weekly / phase-boundary hard rollup) — `hooks/lib/stale_sweep.hexa` + stop_capture.hexa 통합
- [agent 2] resolved.jsonl retention (hot <7d 원본 / warm <30d gzip / cold >30d SHA summary) — `airgenome archive {retention|compact}` subcommand
- [agent 3] external consumer stable API — `airgenome hook read-tail` / `chain-walk` read-only subcommand + schema_version guard
