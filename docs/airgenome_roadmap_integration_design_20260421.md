...
**Status**: P1+P2+P3 전부 landed (2026-04-21 19:00 KST). L5 T*/cp_shift + L6 slash 통합 완료. L2.cert_linker 확장 (13 tool air_chain embed) + L4 os_rollback_cli 도 완료. P4 (L7 장기 연속성) 는 **partial/paused** — 3-agent 병렬 wave 중 fork-storm 위험으로 stop, WIP 2개 worktree 에 보존 (retention CLI/lib, hook_cli_read). 빈 worktree 2개는 prune 완료. 재개는 개별 순차 발사 권장.
...
### P4 (L7 장기 연속성) — partial / paused (2026-04-21 stop)

3-agent 병렬 wave 발사 → fork-storm 위험으로 세 agent 모두 stop. main 에는 landing 없음. WIP 는 airgenome worktree 에 보존.

| agent | 범위 | 진행도 | 잔존 |
|---|---|---|---|
| stale sweep | 20-turn / 24h / 7d / phase-boundary hard rollup (`hooks/lib/stale_sweep.hexa` + stop_capture 통합) | parse clean, selftest 직전 stuck | branch `l7-wip-stale-sweep-20260421` (airgenome, 2 commit: `11705133` lib + `b30b0861` stop_capture integration) |
| retention | resolved.jsonl hot/warm/cold (`airgenome archive {retention\|compact}`) | 문법 debug 미완 ([...] array literal) | worktree `agent-retention` (uncommitted WIP: `hooks/hook_retention_cli.hexa`, `hooks/lib/retention.hexa`, `bin/airgenome` M) |
| read-API | `airgenome hook read-tail` / `chain-walk` read-only + schema_version guard | printf/exit 타이밍 debug 미완 | worktree `agent-readapi` (uncommitted WIP: `hooks/hook_cli_read.hexa`) |

재개 시: worktree 별 개별 순차 (≤1 bg agent), main rebase 후 debug 이어가기. `agent-a0d895c3` / `agent-aff8a635` 는 main 대비 -6266~-7071 line 의 landed 작업 삭제 포함 → 단일 추가물만 pick 하고 branch 는 버릴 것.

완료 시 이 섹션은 Landed (main) 으로 이동.

### 운영 Caveat

- airgenome 에 fork-storm 문제 (`hexa_stage0` shim) 있어 hex-lang `d8f42e62 fix(hexa_stage0): LOCK_WAIT 300→10s + FORK_STORM_CAP=30` 이 먼저 landed. 재발 시 `pkill -9 -f hexa_stage0` + 관련 launchd unload (`launchctl list | grep airgenome.*verdict`).
- **병렬 agent ≤ 2 (rule)**: 3+ 병렬 발사 시 fork-storm 재발 확률 급증. bg subagent 는 순차 발사 권장. P4 L7 wave (3-agent) 가 이 rule 위반으로 중단됨.
- **WIP worktree 보존 정책**: uncommitted WIP 가 있는 worktree (예: `agent-retention`, `agent-readapi`) 는 prune 금지 — branch 에 기록 안 된 파일이 소실됨. 빈 worktree (0 ahead, 0 WIP) 만 prune 대상.
- **stale worktree 취급**: main 대비 거대 음수 diff 인 branch (예: `agent-a0d895c3` -7071L, `agent-aff8a635` -6266L) 는 P1/P2/P3 landed 작업 이전 snapshot 이므로 merge 절대 금지. 추가물만 cherry-pick 후 branch 삭제.
