...
│   ├── **연결 규약: CLI-ONLY** — 모든 외부 consumer 는 `airgenome hook` CLI 로만 연결
│   │   └── state 파일 (.hook-commands/active.json, findings.jsonl, resolved.jsonl) 직접 I/O 금지
│   │       → SSOT=airgenome, consumer=CLI caller, contract 깨끗한 경계
