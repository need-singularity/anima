...
| status | bound | 현재 위치 |
|---|---|---|
| ✗ 미구현 | d=4096 vs d=16 차원 gap × 4096 / 16 = 256× overhead | spec 없음, P_S projector 만 존재 |

**Weakest link**: 16-template eigenvec 가 trained ckpt 의 4096-d hidden 을
충분히 span 하는지의 **실측 evidence 0**. AN11(b) cos>0.5 가 partial proxy.
...
| L1 | ✗ 미구현 | d=4096 → d=16 PCA evidence 0 |
...
2. **L1** — r6 ckpt 실제 hidden 위에 P_S projector 적용한 phi_extractor
   결과를 fixture 와 비교하는 spec (단 1건도 없음).
