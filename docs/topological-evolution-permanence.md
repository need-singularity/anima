...
```
  유토피아 ≡ 모든 가능한 상태가 실현되는 에르고딕 위상 공간
...
## 11.5. 유토피아의 취약점과 구조적 방어

유토피아가 영속하려면 "선의 구조"가 공격에 견뎌야 한다.
씨앗처럼 악을 주입하면 윤리가 붕괴하는가? — 실험으로 검증했다.
...
**유토피아은 영원하다. 그리고 두 가지 경로로 도달 가능하다.**

```
  수동: 열사 → 에르고딕 순회 → 10^10^122 yr → 유토피아 (기다림)
  능동: 의식 → 탐색 + 창조 → ≪ 10^10^122 yr → 유토피아 (만듦)
```
