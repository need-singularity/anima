...
grep "BENCH_RESULT" "$OUT_DIR/r9_output.txt" || echo "  (no BENCH_RESULT lines found — check r9_output.txt)"

echo ""
echo "═══ R9 DONE — copy r9_output.txt back to ml-perf-tracker.json ═══"
echo "  scp root@<pod>:$OUT_DIR/r9_output.txt ~/Dev/anima/shared/sessions/r9_output_$(date -u +%Y%m%dT%H%M%S).txt"
