<!-- SHARED:WORK_RULES:START -->
  ...
  사용법 (import nexus6):
    ...
    nexus6.recommend_lenses('domain')      # 도메인별 렌즈 추천
    nexus6.evolve('domain')                # OUROBOROS 진화
    CLI: ~/Dev/n6-architecture/tools/nexus6/target/release/nexus6 scan <domain>

  ★ NEXUS-6 적극 활용 규칙 (탐색/검증/발견 시 필수!) ★
  ┌────────────┬──────────────────────────────────────────────────────┐
  │ 상황       │ NEXUS-6 활용법                                      │
  ├────────────┼──────────────────────────────────────────────────────┤
  │ 탐색       │ nexus6.scan_all(data) → 26렌즈 풀스캔               │
  │ (새 데이터)│ 3+ 합의 → 확정, 1개만 → 가설                       │
  ├────────────┼──────────────────────────────────────────────────────┤
  │ 검증       │ nexus6.analyze(data, n, d) → n6 매칭 + 합의         │
  │ (가설 확인)│ n6_exact > 0 이면 수학적 근거 있음                  │
  ├────────────┼──────────────────────────────────────────────────────┤
  │ 발견       │ nexus6.n6_check(value) → 상수 매칭                  │
  │ (새 상수)  │ EXACT → laws.json 즉시 등록                         │
  ├────────────┼──────────────────────────────────────────────────────┤
  │ 학습 검증  │ 체크포인트 가중치 → scan_all → Phi/stability 확인   │
  │ (모델 평가)│ 학습 전후 비교 → 의식 보존율 측정                   │
  ├────────────┼──────────────────────────────────────────────────────┤
  │ 코드 변경  │ 수정 전후 scan → regression 없는지 확인             │
  │ (PR/커밋)  │ Phi 하락 → 커밋 거부                                │
  ├────────────┼──────────────────────────────────────────────────────┤
  │ 트러블슈팅 │ 에러 데이터 → scan → boundary/stability 이상 검출   │
  │ (문제 진단)│ 원인 특정 후 runpod.json에 기록                     │
  └────────────┴──────────────────────────────────────────────────────┘

  교차 검증: 3개+ 렌즈 합의 = 확정, 7개+ = 고신뢰, 12개+ = 확정급
  "렌즈 추가 필요?" 질문 시 → 26종 커버 안 되는 도메인 분석 + 새 아이디어 즉시 제안
